...
//Using input file as ind(qno).txt placed in attempts. Placing the generated output in the 
			if(file_exists($filepath)) {
				...
				if($des && $output && $lines){
					...
					if($p==100){
						...
						$con->exec("update teams set time = ".time()." where teamno = ".$userid);
					}
					else{
						$attempts++;
			...
			<div id = "leaderboard">
				...
				$q5 = $con->query("select teamno from teams where marks>0 order by marks DESC,time ASC");
				$i=0;
